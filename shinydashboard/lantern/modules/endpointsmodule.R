...
endpointsmodule <- function(
  ...
  output$endpoints_table <- reactable::renderReactable({
     reactable(
              selected_fhir_endpoints() %>% select(url, endpoint_names, vendor_name, fhir_version, format, status, availability) %>% distinct(url, endpoint_names, vendor_name, fhir_version, format, status, availability) %>% group_by(url) %>% mutate_all(as.character),
